...
const PLUGIN_ROOT = "./plugins/ot-aio/"
const STORAGE_ROOT = "./plugins/ot-aio/storage/"
const STICKY_DATABASE_CATEGORY = "ot-aio:sticky:sticky"
...
const isStickyModuleEnabled = () => {
    const config = opendiscord.configs.get("ot-aio:config") as any
    return config?.data?.modules?.sticky?.enabled !== false
}
    ...
    renderEmbed(embed: OTStickyEmbedData, guild: discord.Guild | null, channel: discord.GuildTextBasedChannel | null): OTStickyEmbedData {
        return {
            ...
            fields: Array.isArray(embed.fields) ? embed.fields.map((field) => ({
                name: this.renderText(field.name, guild, channel),
                value: this.renderText(field.value, guild, channel),
...
class OTStickyManager extends api.ODManagerData {
    ...
    get config(): OTStickyConfig {
        return opendiscord.configs.get("ot-aio:sticky:config") as OTStickyConfig
    }

    get database(): OTStickyDatabase {
        return opendiscord.databases.get("ot-aio:sticky:database") as OTStickyDatabase
    }
    ...
    getStoredAttachmentPath(attachmentData: OTStickyAttachmentData | null) {
        if (!attachmentData?.storedName) return null

        const storageRoot = path.resolve(STORAGE_ROOT)
        const fullPath = path.resolve(storageRoot, attachmentData.storedName)
        if (fullPath === storageRoot || !fullPath.startsWith(storageRoot + path.sep)) return null

        return fullPath
    }

    deleteAttachmentFile(attachmentData: OTStickyAttachmentData | null) {
        const fullPath = this.getStoredAttachmentPath(attachmentData)
    ...
    buildStickyMessage(entry: OTStickyEntry, channel: discord.GuildTextBasedChannel): discord.MessageCreateOptions {
        ...
        if (entry.embedData) {
            ...
            if (renderedEmbed.timestamp) embed.setTimestamp()
            if (renderedEmbed.fields?.length) embed.setFields(renderedEmbed.fields.slice(0, 25))
        ...
        if (entry.attachmentData) {
            if (entry.attachmentData.fileUrl) {
                ...
                const attachmentPath = this.getStoredAttachmentPath(entry.attachmentData)
                if (!attachmentPath || !fs.existsSync(attachmentPath)) throw new api.ODPluginError(`Missing stored sticky attachment: ${entry.attachmentData.storedName}`)
    ...
    async handleMessage(message: discord.Message) {
        if (!isStickyModuleEnabled()) return
    ...
    async handleDashboardRequest(req: http.IncomingMessage, res: http.ServerResponse, password: string) {
        ...
        if (stickyMatch) {
            ...
            if (req.method === "PUT") {
                ...
                const entryType = (["text", "embed", "attachment"].includes(body.type) ? body.type : "text") as OTStickyType
                const attachmentData = entryType === "attachment"
                    ? (body.attachmentUrl ? { storedName: "", originalName: "attachment", contentType: null, spoiler: false, fileUrl: String(body.attachmentUrl) } : (existing?.attachmentData ?? null))
                    : null
                ...
                this.saveEntry(entry)
                if (existing?.attachmentData && existing.attachmentData !== attachmentData) this.deleteAttachmentFile(existing.attachmentData)
...
declare module "#opendiscord-types" {
    ...
    export interface ODConfigManagerIds_Default {
        "ot-aio:sticky:config": OTStickyConfig
    }

    export interface ODDatabaseManagerIds_Default {
        "ot-aio:sticky:database": OTStickyDatabase
    }

    export interface ODCheckerManagerIds_Default {
        "ot-aio:sticky:config": api.ODChecker
    }

    export interface ODPluginClassManagerIds_Default {
        "ot-aio:sticky:manager": OTStickyManager
    }

    export interface ODSlashCommandManagerIds_Default {
        "ot-aio:sticky:sticky": api.ODSlashCommand
    }

    export interface ODCommandResponderManagerIds_Default {
        "ot-aio:sticky:sticky": { source: "slash" | "text", params: {}, workers: "ot-aio:sticky:sticky" | "ot-aio:sticky:logs" }
    }

    export interface ODEmbedManagerIds_Default {
        "ot-aio:sticky:reply-embed": { source: "slash" | "text" | "other", params: { data: OTStickyReplyData }, workers: "ot-aio:sticky:reply-embed" }
    }

    export interface ODMessageManagerIds_Default {
        "ot-aio:sticky:reply-message": { source: "slash" | "text" | "other", params: { data: OTStickyReplyData }, workers: "ot-aio:sticky:reply-message" }
    }
}
...
const getStickyManager = () => opendiscord.plugins.classes.get("ot-aio:sticky:manager") as OTStickyManager
...
opendiscord.events.get("onPluginClassLoad").listen((classes) => {
    classes.add(new OTStickyManager("ot-aio:sticky:manager"))
})

// REGISTER CONFIG
opendiscord.events.get("onConfigLoad").listen((configs) => {
    configs.add(new OTStickyConfig("ot-aio:sticky:config", "sticky.config.json", PLUGIN_ROOT))
})

// REGISTER DATABASE
opendiscord.events.get("onDatabaseLoad").listen((databases) => {
    databases.add(new OTStickyDatabase("ot-aio:sticky:database", "sticky.database.json", stickyDatabaseFormatter, PLUGIN_ROOT))
})

// REGISTER CONFIG CHECKER
opendiscord.events.get("onCheckerLoad").listen((checkers) => {
    const config = opendiscord.configs.get("ot-aio:sticky:config")
    const structure = new api.ODCheckerObjectStructure("ot-aio:sticky:config", {
        children: [
            { key: "commandPermission", optional: false, priority: 0, checker: new api.ODCheckerStringStructure("ot-aio:sticky:command-permission", { choices: ["admin", "developer", "owner"] }) },
            { key: "autoResendOnBoot", optional: false, priority: 0, checker: new api.ODCheckerBooleanStructure("ot-aio:sticky:auto-resend", {}) },
            { key: "deleteStickyMessageWhenDisabled", optional: false, priority: 0, checker: new api.ODCheckerBooleanStructure("ot-aio:sticky:delete-disabled", {}) },
            {
                key: "dashboard", optional: true, priority: 0, checker: new api.ODCheckerObjectStructure("ot-aio:sticky:dashboard", {
                    children: [
                        { key: "enabled", optional: true, priority: 0, checker: new api.ODCheckerBooleanStructure("ot-aio:sticky:dashboard-enabled", {}) },
                        { key: "host", optional: true, priority: 0, checker: new api.ODCheckerStringStructure("ot-aio:sticky:dashboard-host", { minLength: 1, maxLength: 128 }) },
                        { key: "port", optional: true, priority: 0, checker: new api.ODCheckerNumberStructure("ot-aio:sticky:dashboard-port", { min: 1, max: 65535, floatAllowed: false }) },
                        { key: "password", optional: true, priority: 0, checker: new api.ODCheckerStringStructure("ot-aio:sticky:dashboard-password", { minLength: 0, maxLength: 256 }) }
                    ]
                })
            }
        ]
    })
    checkers.add(new api.ODChecker("ot-aio:sticky:config", checkers.storage, 0, config!, structure))
})
...
opendiscord.events.get("onSlashCommandLoad").listen((slash) => {
    ...
    slash.add(new api.ODSlashCommand("ot-aio:sticky:sticky", {
        name: "sticky",
...
opendiscord.events.get("onHelpMenuComponentLoad").listen((menu) => {
    menu.get("opendiscord:extra").add(new api.ODHelpMenuCommandComponent("ot-aio:sticky:sticky", 0, {
        slashName: "sticky",
...
opendiscord.events.get("onEmbedBuilderLoad").listen((embeds) => {
    embeds.add(new api.ODEmbed("ot-aio:sticky:reply-embed"))
    embeds.getSafe("ot-aio:sticky:reply-embed").workers.add(
        new api.ODWorker("ot-aio:sticky:reply-embed", 0, (instance: any, params: any) => {
...
opendiscord.events.get("onMessageBuilderLoad").listen((messages) => {
    messages.add(new api.ODMessage("ot-aio:sticky:reply-message"))
    messages.getSafe("ot-aio:sticky:reply-message").workers.add(
        new api.ODWorker("ot-aio:sticky:reply-message", 0, async (instance: any, params: any, source: any) => {
            instance.addEmbed(await opendiscord.builders.embeds.getSafe("ot-aio:sticky:reply-embed").build(source, params))
...
opendiscord.events.get("onCommandResponderLoad").listen((commands) => {
    ...
    commands.add(new api.ODCommandResponder("ot-aio:sticky:sticky", generalConfig.data.prefix, "sticky"))
    commands.get("ot-aio:sticky:sticky")!.workers.add([
        new api.ODWorker("ot-aio:sticky:sticky", 0, async (instance: any, params: any, source: any, cancel: any) => {
            const { guild, channel, user } = instance

            if (!isStickyModuleEnabled()) {
                instance.reply(await opendiscord.builders.messages.getSafe("ot-aio:sticky:reply-message").build(source, {
                    data: buildReply("Sticky Messages Disabled", "The sticky messages module is disabled in the AIO dashboard.", [], "Red")
                }))
                return cancel()
            }
        ...
        new api.ODWorker("ot-aio:sticky:logs", -1, (instance: any, params: any, source: any) => {
